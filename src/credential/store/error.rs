...
impl fmt::Display for StoreError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Io { source: err } => write!(f, "cache IO error: {err}"),
            Self::Json { source: err } => write!(f, "cache JSON error: {err}"),
...
impl std::error::Error for StoreError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Self::Io { source: err } => Some(err),
            Self::Json { source: err } => Some(err),
            Self::InsecurePath { .. }
            | Self::InvalidKey(_)
            | Self::InconsistentMetadata { .. }
            | Self::UnexpectedKind { .. }
...
impl From<crate::run::lifecycle::RunLifecycleError> for StoreError {
